...
def _check_game_pks(game_pks: list) -> None:
    if game_pks is None:
        raise ValueError(
    ...
    for game, game_info in game_pks.items():
        ...
        if not all(key in game_info for key in ['home', 'away']):
            raise ValueError(f"Game entry '{game}' must contain both 'home' and 'away' keys.")
...
def thread_game_dates(start_dt: date, end_dt: date, team_abbr: str = None) -> list:
...
def thread_game_data(game_pks: list, player: str, pitch_type: str, max_videos_per_game: int, max_return_videos: int) -> pl.DataFrame:
    play_ids_df = []
    with ProgressBar(len(game_pks), desc='Extracting Game Data') as progress:
        with concurrent.futures.ThreadPoolExecutor(max_workers=cpu_threads) as executor:
...
# THis one extracts where the game Ids are known
@overload
def get_pbp_data(
