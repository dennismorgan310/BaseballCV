...
def create_concatenated_video(selected_rows: pd.DataFrame):
    ...
    try:
        ...
        try:
            ...
            print(f"DEBUG: Error cleaning up temp directory: {e}")import os
import io
import zipfile
import streamlit as st
import pandas as pd
...
def create_concatenated_video(selected_rows: pd.DataFrame):
    """
    Downloads videos and concatenates them using ffmpeg.
    Returns the concatenated video as bytes for download.
    """
    # Check if ffmpeg is available
    if not check_ffmpeg_available():
        raise Exception(
            "FFmpeg is required for video concatenation but is not installed. "
            "Please install FFmpeg or use 'Individual videos' option instead. "
    ...
    try:
        ...
        for i, row in enumerate(selected_rows.itertuples()):
            progress_text = f"Downloading video {i+1}/{total_videos}: {row.batter_name} vs {row.pitcher_name}"
            progress_bar.progress((i + 1) / (total_videos + 1), text=progress_text)
        ...
        progress_bar.progress(0.9, text="Concatenating videos with ffmpeg...")
        ...
        try:
            ...
            print("DEBUG: Copy failed, trying re-encoding...")
            ffmpeg_cmd = [
                'ffmpeg',
                '-f', 'concat',
                '-safe', '0', 
...
# Optional: Simple concatenation fallback without any external dependencies
def create_simple_concatenated_video(selected_rows: pd.DataFrame):
    ...
    Use this if neither moviepy nor ffmpeg are available.
    """
    st.warning("⚠️ Video concatenation requires FFmpeg. Creating ordered zip file instead.")
